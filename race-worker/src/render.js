...
export const BANNER_DIM = [70, 60, 30]; // finish banner before anyone has won

// row layout: finish banner, then 4 three-row lanes separated by one dark row each
const LANE_HEIGHT = 3;
export const LANE_STARTS = SCHOOLS.map((_, i) => 1 + i * (LANE_HEIGHT + 1)); // [1, 5, 9, 13]
...
export function buildFrame({ progressCols, status, winner }) {
  ...
  SCHOOLS.forEach((school, i) => {
    ...
    drawLaneMascot(grid, school, top, progress); // the mascot rides the head of its bar
  });

  // finish banner: dim always-on marker, brighter + winner-colored once someone's won
  const finishColor =
    status === "finished" && winner ? softenColor(SCHOOL_INFO[winner].color) : BANNER_DIM;
